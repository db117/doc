...
> 定时调度的线程池，继承与`ThreadPoolExecutor`。通过重写`submit`，`execute`等方法，并实现私有的`BlockingQueue`：
`java.util.concurrent.ScheduledThreadPoolExecutor.DelayedWorkQueue`。把所有的的任务都包装成
`java.util.concurrent.ScheduledThreadPoolExecutor.ScheduledFutureTask`来进行统一管理。
...
> 一个私有的无界阻塞队列，针对 ScheduledThreadPool 进行特殊处理。主要表现在获取数据时通过`ScheduledFutureTask#getDelay`
> 来处理。
